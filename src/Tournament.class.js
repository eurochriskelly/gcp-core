...
class Tournament {
    ...
    get categoryTeams() {
        return this.categoryNames.reduce((teams, name) => {
            teams[name] = Object.values(this.categories[name])
                .reduce((p, n) => [...p, ...n], [])
            return teams
        }, {})
    }
    ...
    addTeam(teamName, category, group = 0) {
        ...
        if (!group) {
            ...
            const lastGroup = this.categories[lastGroupKey]
            if (lastGroup.length > 5) {
                const numGroups = categoryGroups.length
                this.categories[`g${numGroups + 1}`]
            }
        } else {
    ...
    addFixture(time, pitch, stage, category, group, team1, team2, umpireTeam, duration) {
        this.schedules.push([
            time, pitch, stage, category, group, 
            team1, team2, umpireTeam, duration 
        ])
    }
    load(tdata) {
        const {
            tournamentId, description, startDate,
            pitches, categories, schedule
        } = tdata
